    ...
    return (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.45)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
            <div style={{ background: "#fff", borderRadius: 16, padding: "28px 24px", width: "100%", maxWidth: 440, boxShadow: "0 24px 60px rgba(0,0,0,0.15)", border: BORDER, fontFamily: FONT }} onClick={(e) => e.stopPropagation()}>
                <h2 style={{ margin: "0 0 20px", fontSize: 18, fontWeight: 700, color: "#1a1a1a" }}>{mode === "edit" ? "Edit Vehicle Type" : "Add Vehicle Type"}</h2>
                ...
                <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                    ...
                    <button onClick={onSubmit} style={{ padding: "9px 20px", borderRadius: 10, border: "none", background: BRAND, fontSize: 14, fontWeight: 600, color: "#fff", cursor: "pointer", fontFamily: FONT }}>{mode === "edit" ? "Save Changes" : "Add Vehicle Type"}</button>
                </div>
            </div>
        </div>
    );
...
function VehicleTypesTable({ items, onEdit, onArchive, loading, error }) {
    ...
    return (
        <div style={{ background: "#fff", borderRadius: 12, border: BORDER, overflow: "hidden", fontFamily: FONT }}>
            {items.length === 0 ? <div style={{ padding: "28px 20px", color: "#888", fontSize: 14 }}>No vehicle types found.</div> : (
                <>
...
export default function ConfigurationVehicleTypes({ vehicleTypes, loading, error, search, onSearchChange, showArchived, onToggleArchived, onReload }) {
    ...
    const filtered = useMemo(() => vehicleTypes.filter((v) => v.name.toLowerCase().includes(search.toLowerCase()) && (showArchived || !v.archived)).sort((a, b) => b.id - a.id), [vehicleTypes, search, showArchived]);
    ...
    const confirmArchive = async () => {
        try { await archiveVehicleType(archiveModal.item.id); await onReload(); }
        ...
        <>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, fontFamily: FONT }}>
                <div style={{ display: "flex", alignItems: "center", height: 42, width: 280, borderRadius: 10, border: BORDER, padding: "0 14px", background: "#fff", boxSizing: "border-box", gap: 8 }}>
                    ...
                    <input placeholder="Search vehicle types..." value={search} onChange={(e) => onSearchChange(e.target.value)} style={{ border: "none", outline: "none", width: "100%", fontSize: 13, background: "transparent", color: "#1a1a1a", fontFamily: FONT }} />
                </div>
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                    ...
                    <button onClick={openCreate} style={{ height: 42, borderRadius: 10, padding: "0 18px", background: BRAND, color: "#fff", border: "none", fontWeight: 600, fontSize: 13, cursor: "pointer", fontFamily: FONT, boxShadow: "0 2px 8px rgba(23,168,74,0.2)" }}>+ Add Vehicle Type</button>
                </div>
            </div>
