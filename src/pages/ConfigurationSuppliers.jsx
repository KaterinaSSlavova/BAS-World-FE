    ...
    return (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.45)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
            <div style={{ background: "#fff", borderRadius: 18, padding: "32px 28px", width: "100%", maxWidth: 480, boxShadow: "0 20px 60px rgba(15,23,42,0.18)" }} onClick={(e) => e.stopPropagation()}>
                <h2 style={{ margin: "0 0 24px", fontSize: 22, fontWeight: 800, color: "#1f2937" }}>{mode === "edit" ? "Edit Supplier" : "Add Supplier"}</h2>
                <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
                    <div style={{ width: 96, height: 96, borderRadius: 16, border: "2px dashed #d9dee5", background: "#f8fafc", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                        {pictureUrl ? <img src={pictureUrl} alt="preview" style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={(e) => { e.target.style.display = "none"; }} /> : <span style={{ fontSize: 32 }}>🏢</span>}
                    </div>
                </div>
                ...
                <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
                    ...
                    <button onClick={handleSubmit} disabled={uploading} style={{ padding: "12px 22px", borderRadius: 10, border: "none", background: uploading ? "#a3d9b8" : "#2e9d5b", fontSize: 15, fontWeight: 700, color: "#fff", cursor: uploading ? "not-allowed" : "pointer" }}>{mode === "edit" ? "Save Changes" : "Add Supplier"}</button>
                </div>
            </div>
        </div>
    );
...
function SupplierCard({ supplier, onEdit, onArchive }) {
    ...
    return (
        <div style={{ background: "#fff", borderRadius: 16, border: `1px solid ${expanded ? "#b9dec6" : "#e6eaef"}`, overflow: "hidden", transition: "border-color 0.15s", opacity: supplier.archived ? 0.6 : 1 }}>
            <div onClick={() => setExpanded((v) => !v)} style={{ padding: "20px 18px 16px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                <div style={{ width: 72, height: 72, borderRadius: 14, background: "#f1f3f6", border: "1px solid #e6eaef", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                    {supplier.picture ? <img src={supplier.picture} alt={supplier.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ fontSize: 28, color: "#b0b8c4" }}>🏢</span>}
                </div>
                <div style={{ textAlign: "center" }}>
                    <div style={{ fontSize: 16, fontWeight: 800, color: "#273142" }}>{supplier.name}</div>
                    {supplier.archived && <span style={{ display: "inline-block", marginTop: 6, padding: "3px 10px", borderRadius: 999, background: "#fff7e8", color: "#d97706", border: "1px solid #f5d29c", fontSize: 12, fontWeight: 700 }}>Archived</span>}
                </div>
            ...
            {expanded && (
                <div style={{ borderTop: "1px solid #eef1f4", padding: "14px 18px 18px", display: "flex", flexDirection: "column", gap: 10 }}>
                    <div style={{ fontSize: 13, color: "#7b8494" }}><span style={{ fontWeight: 700 }}>ID:</span> #{supplier.id}</div>
                    {supplier.picture && (
...
export default function ConfigurationSuppliers({ suppliers, loading, error, search, onSearchChange, showArchived, onToggleArchived, onReload }) {
    ...
    const filtered = useMemo(() => suppliers
        ...
        .sort((a, b) => b.id - a.id), [suppliers, search, showArchived]);
        ...
        <>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                ...
                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                    ...
                    <button onClick={openCreate} style={{ height: 50, borderRadius: 12, padding: "0 22px", background: "#2e9d5b", color: "#fff", border: "none", fontWeight: 700, fontSize: 15, cursor: "pointer", boxShadow: "0 4px 12px rgba(46,157,91,0.18)", whiteSpace: "nowrap" }}>+ Add Supplier</button>
                </div>
            </div>
            {loading ? <div style={{ padding: 24, color: "#7f8792", fontSize: 15 }}>Loading...</div>
                : error ? <div style={{ padding: 24, color: "#d14343", fontSize: 15 }}>{error}</div>
                    : filtered.length === 0 ? <div style={{ padding: 24, color: "#7f8792", fontSize: 15 }}>No suppliers found.</div>
