...
import { createVehicleType, updateVehicleType, archiveVehicleType } from "../lib/api/vehicleTypes.ts";
        ...
        <div style={{
            ...
            <div style={{
                ...
                <h2 style={{ margin: "0 0 24px", fontSize: 22, fontWeight: 800, color: "#1f2937" }}>
                    {mode === "edit" ? "Edit Vehicle Type" : "Add Vehicle Type"}
                </h2>
                <label style={{
                ...
                <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
                    ...
                    }}>{mode === "edit" ? "Save Changes" : "Add Vehicle Type"}</button>
                </div>
            </div>
        </div>
    );
...
function VehicleTypesTable({ items, onEdit, onArchive, loading, error }) {
    ...
    return (
        <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #e6eaef", overflow: "hidden" }}>
            {items.length === 0 ? (
                <div style={{ padding: "32px 24px", color: "#7f8792", fontSize: 15 }}>No vehicle types found.</div>
            ) : (
                <>
                    <div style={{
...
export default function ConfigurationVehicleTypes({ vehicleTypes, loading, error, search, onSearchChange, showArchived, onToggleArchived, onReload }) {
    ...
    const filtered = useMemo(() => vehicleTypes
            ...
            .sort((a, b) => b.id - a.id),
        [vehicleTypes, search, showArchived]);
    ...
    const confirmArchive = async () => {
        try {
            ...
            console.error(err);
            alert("Failed to archive vehicle type.");
        } finally {
    ...
    return (
        <>
            ...
            <VehicleTypesTable items={filtered} onEdit={openEdit}
            ...
            <ConfirmArchiveModal open={archiveModal.open} item={archiveModal.item}
                                 entityLabel="Vehicle Type" onConfirm={confirmArchive}
