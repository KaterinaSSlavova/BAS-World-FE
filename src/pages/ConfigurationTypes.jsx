    ...
    return (
        <div style={{ display: "flex", gap: 6, justifyContent: "center", alignItems: "center", marginTop: 8 }}>
            <button onClick={() => onPage(page - 1)} disabled={page === 0} style={{
                ...
                color: page === 0 ? "#c4c9d2" : "#374151", cursor: page === 0 ? "default" : "pointer",
            }}>&#8249;</button>
            {Array.from({ length: totalPages }, (_, i) => (
                <button key={i} onClick={() => onPage(i)} style={{
                    ...
                    fontWeight: 700, fontSize: 14, color: i === page ? "#fff" : "#374151", cursor: "pointer",
                }}>{i + 1}</button>
            ))}
            <button onClick={() => onPage(page + 1)} disabled={page === totalPages - 1} style={{
                padding: "8px 14px", borderRadius: 9, border: "1px solid #d9dee5",
                background: "#fff", fontWeight: 700, fontSize: 14,
                color: page === totalPages - 1 ? "#c4c9d2" : "#374151", cursor: page === totalPages - 1 ? "default" : "pointer",
            }}>&#8250;</button>
        </div>
    );
}
    ...
    return (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.45)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
            <div style={{ background: "#fff", borderRadius: 18, padding: "32px 28px", width: "100%", maxWidth: 460, boxShadow: "0 20px 60px rgba(15,23,42,0.18)" }} onClick={(e) => e.stopPropagation()}>
                <h2 style={{ margin: "0 0 24px", fontSize: 22, fontWeight: 800, color: "#1f2937" }}>{mode === "edit" ? "Edit Type" : "Add Type"}</h2>
                ...
                <input autoFocus value={value} onChange={(e) => onChange(e.target.value)} placeholder="Enter type name..." onKeyDown={(e) => e.key === "Enter" && onSubmit()}
                ...
                <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
                    ...
                    <button onClick={onSubmit} style={{ padding: "12px 22px", borderRadius: 10, border: "none", background: "#2e9d5b", fontSize: 15, fontWeight: 700, color: "#fff", cursor: "pointer" }}>{mode === "edit" ? "Save Changes" : "Add Type"}</button>
                </div>
            </div>
        </div>
    );
...
function TypesTable({ items, onEdit, onArchive, loading, error }) {
    ...
    return (
        <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #e6eaef", overflow: "hidden" }}>
            {items.length === 0 ? <div style={{ padding: "32px 24px", color: "#7f8792", fontSize: 15 }}>No types found.</div> : (
                <>
...
export default function ConfigurationTypes({ types, loading, error, search, onSearchChange, showArchived, onToggleArchived, onReload }) {
    ...
    const confirmArchive = async () => {
        try {
            await archiveType(archiveModal.item.id);
            await onReload();
        ...
        <>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                ...
                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                    ...
                    <button onClick={openCreate} style={{ height: 50, borderRadius: 12, padding: "0 22px", background: "#2e9d5b", color: "#fff", border: "none", fontWeight: 700, fontSize: 15, cursor: "pointer", boxShadow: "0 4px 12px rgba(46,157,91,0.18)", whiteSpace: "nowrap" }}>+ Add Type</button>
                </div>
            </div>
