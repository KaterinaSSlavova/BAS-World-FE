    ...
    return (
        <AppLayout>
            ...
            <div style={{
                ...
                <StatCard label="ACTIVE PRODUCTS" value={String(activeCount)} sub="Currently available" subColor="#2e9d5b" />
                <StatCard label="UNAVAILABLE" value={String(unavailableCount)} sub="Not available" subColor="#d14343" />
                <StatCard label="LOW STOCK" value={String(lowStockCount)} sub="Needs attention" subColor="#d97706" />
                <StatCard label="INVENTORY VALUE" value={formatPrice(inventoryValue)} sub="Across all products" subColor="#7f8792" />
            </div>

            {/* Bottom panels */}
            <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 20 }}>
                {/* Recent Products */}
                <div style={{ flex: 2, background: "#fff", border: "1px solid #e6eaef", borderRadius: 18, overflow: "hidden" }}>
                    <div style={{ padding: "20px 24px", borderBottom: "1px solid #eef1f4" }}>
                        <h2 style={{ fontSize: 15, fontWeight: 700, color: "#1f2937", margin: 0 }}>Recent Products</h2>
                    </div>

                    {isMobile ? (
                        <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: 12 }}>
                            {products.slice(0, 8).map((p) => (
                                <div key={p.sku} style={{ border: "1px solid #eef1f4", borderRadius: 12, padding: "12px 14px", display: "flex", flexDirection: "column", gap: 6 }}>
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                                        <div style={{ fontSize: 14, fontWeight: 700, color: "#273142", flex: 1, marginRight: 8 }}>{p.name}</div>
                                        <span style={{
                        ...
                        <>
                            <div style={{
                                ...
                                padding: "12px 24px", background: "#fbfcfd", borderBottom: "1px solid #eef1f4",
                                fontSize: 11, fontWeight: 800, color: "#7b8494", textTransform: "uppercase", letterSpacing: "0.08em",
                            }}>
                ...
                {/* Depot Overview */}
                <div style={{ flex: isMobile ? "unset" : 1, background: "#fff", border: "1px solid #e6eaef", borderRadius: 18, overflow: "hidden" }}>
                    <div style={{ padding: "20px 24px", borderBottom: "1px solid #eef1f4" }}>
                        <h2 style={{ fontSize: 15, fontWeight: 700, color: "#1f2937", margin: 0 }}>Depot Overview</h2>
                    </div>
                    <div style={{
                        padding: "12px 16px",
                        ...
                        {depots.map((d) => (
                            <div key={d.depotName} style={{ border: "1px solid #f0f4f0", borderRadius: 10, padding: "14px 16px" }}>
                                <div style={{ fontSize: 14, fontWeight: 700, color: "#273142", marginBottom: 4 }}>{d.depotName}</div>
                                <div style={{ fontSize: 12, color: "#7f8792", marginBottom: 8 }}>📍 {d.location}</div>
                                <div style={{ fontSize: 12, color: "#555", fontWeight: 500 }}>◈ {d.numberOfProducts} products</div>
                            </div>
                        ))}
                    </div>
                </div>
