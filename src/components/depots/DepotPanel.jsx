...
export default function DepotPanel({ depotId, depot, onClose, onUpdated, onArchived }) {
    ...
    const totalStock = products.reduce((sum, pd) => {
        const depotInfo = pd.depots?.find(
            (d) => String(getDepotId(d)) === String(depotId)
        );
    ...
    useEffect(() => {
        ...
        getAllProductDepots()
            .then((data) => {
                console.log("product depots:", data);
    ...
    const handleArchive = async () => {
        if (!window.confirm(`Archive "${depot.depotName}"? This cannot be undone.`)) {
            return;
        }
            ...
            >
                ...
                >
                    <Metric
                        ...
                        sub="product types stocked"
                    />

                    <Metric
                        label="Available"
                        value={availableCount}
                        sub={`${unavailableCount} unavailable`}
                    />
                                        ...
                                        <td style={tdStyle}>
                                                <span
                                                    style={{
                                                        ...
                                                        background: depotInfo?.available
                                                            ? "#eefaf2"
                                                            : "#fff0f0",
                                                        color: depotInfo?.available
                                                            ? "#1a7a40"
                                                            : "#b91c1c",
                                                    }}
                                                >
...
function Metric({ label, value, sub }) {
    return (
        ...
        >
            <div style={{ fontSize: 11, color: "#7b8494" }}>
                {label}
            </div>
